---
type: 'vendor'
title: 'Architectural Approaches for Identity in Multitenant Solutions - Azure Architecture Center'
source_url: 'https://learn.microsoft.com/en-us/azure/architecture/guide/multitenant/approaches/identity'
vendor: ['azure']
industry: []
data_stack: []
cloud: ['azure']
...
[Edit](https://github.com/microsoftdocs/architecture-center/blob/main/docs/guide/multitenant/approaches/identity.md)
...
Before you start to build an identity system for a multitenant solution, see [Architectural considerations for identity in a multitenant solution](../considerations/identity).
...
For more information, see the [Federated identity pattern](../../../patterns/federated-identity).
...
Also consider providing federation as a feature that only applies to customers at a higher [product tier](../considerations/pricing-models#feature--and-service-level-based-pricing).
...
Some identity platforms support impersonation, either as a built-in feature or by using custom code. For example, you can [add a custom claim in Microsoft Entra External ID](/en-us/entra/external-id/customers/concept-custom-extensions#token-issuance-start-event) for the impersonated user ID, or you can replace the subject identifier claim in the issued tokens.
...
- **Your IdP:** For example, if you use Microsoft Entra ID as your IdP, you can use features like [app roles](/en-us/entra/identity-platform/howto-add-app-roles-in-apps) and [groups](/en-us/entra/fundamentals/how-to-manage-groups) to store authorization information. Your application can use the associated token claims to enforce your authorization rules.
...
- If your tenants or customers are organizations, they might already use Microsoft Entra ID for services like Microsoft 365, Microsoft Teams, or for their own Azure environments. You can create a [multitenant application](/en-us/entra/identity-platform/single-and-multi-tenant-apps) in your own Microsoft Entra ID directory to make your solution available to other Microsoft Entra ID directories. You can also list your solution in [Microsoft Marketplace](/en-us/partner-center/marketplace-offers/plan-saas-offer) so that organizations that use Microsoft Entra ID can discover and acquire it.

- If your tenants or customers don't use Microsoft Entra ID, or if they're individuals instead of organizations, you can use External ID. External ID provides features to control how users sign up and sign in. For example, you can restrict access to your solution to only invited users, or you can turn on self-service sign-up. You can use [custom branding](/en-us/entra/external-id/customers/how-to-customize-branding-customers). To invite your own staff to sign in, use guest access to [add External ID users from your Microsoft Entra ID tenant](/en-us/entra/external-id/b2b-quickstart-add-guest-users-portal). External ID also supports [federation with other IdPs](/en-us/entra/external-id/customers/concept-authentication-methods-customers).
...
Understand your tenants' identity requirements before you finalize your identity system design. For more information about specific requirements, see [Architectural considerations for identity in a multitenant solution](../considerations/identity).
...
You can track the tenant context of a user or token in any [tenancy model](../considerations/tenancy-models) because a user identity always has a tenant context within a multitenant solution. It's a good practice to track tenant context when you deploy independent stamps for a single tenant, which future-proofs your codebase for other forms of multitenancy.
...
- [Architectural considerations for identity in a multitenant solution](../considerations/identity)
