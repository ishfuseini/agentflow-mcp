...
source_url: 'https://cloud.google.com/bigquery/docs/column-level-security-intro'
vendor: ['gcp']
industry: []
data_stack: ['bigquery']
cloud: ['gcp']
...
BigQuery provides fine-grained access to sensitive columns using either *policy tags* (based on Data Catalog) or *data governance tags* (based
on Resource Manager tags). Using these classification methods, you can create
policies that check, at query time, whether a user has proper access. For
example, a policy can enforce access checks such as:

- You must be in `group:high-access` to see the columns containing `TYPE_SSN`.

To enhance column-level access control, you can optionally use [dynamic data masking](/bigquery/docs/column-data-masking-intro).
...
![Workflow](/static/bigquery/images/column-level-security-workflow.png)

To restrict data access at the column level:

1. **Define a taxonomy and policy tags**. Create and manage a taxonomy and
policy tags for your data. For guidelines, see [Best practices for policy tags](/bigquery/docs/best-practices-policy-tags).
...
4. **Manage access on the policy tags**. Use [Identity and Access Management](/iam) (IAM) policies to restrict access to each policy tag. The policy
...
**Note:** Column-level access control is enforced *in addition* to existing [dataset ACLs](/bigquery/docs/dataset-access-controls). A user needs both dataset permission and policy tag permission in
order to access data protected by column-level access control.
...
where [sensitive and high-risk data](/sensitive-data-protection/docs/sensitivity-risk-calculation) reside.
Sensitive Data Protection reports these metrics at the project, table, and column
levels. For more information, see [Overview of sensitive data discovery](/sensitive-data-protection/docs/data-profiles).

The following image shows a list of column data profiles (click to enlarge).
Columns with high data-risk values might contain [high-sensitivity data](/sensitive-data-protection/docs/sensitivity-risk-calculation#high-sensitivity) and have no
...
[![Column data profiles](/static/sensitive-data-protection/docs/images/column-data-profiles.png)](/static/sensitive-data-protection/docs/images/column-data-profiles.png)

*Column data profiles in Sensitive Data Protection*
...
![Policy tags](/static/bigquery/images/policy-tags.png)

To set up column level security, a data steward, who has the [appropriate permissions](#roles), would perform the following steps to set up
a [hierarchy of data classification](/bigquery/docs/best-practices-policy-tags).
...
3. The data steward creates more levels of nodes in the taxonomy, under **High** and **Medium**. The lowest level node is a leaf node,
...
In this example, the data steward assigns the **High** access policy to the *employee_ssn* column in a table.

5. In the **Current schema** page of the console, the data steward can see the
policy tag that governs a particular column. In this example, the **employee_ssn** column is under the **High** policy tag, so when viewing the
...
the policy tag in the `Policy tags` field: `Business criticality:High`.
   ...
   ![Policy tag UI](/static/bigquery/images/schema-ui-policy-tags3.png)
...
For details about these steps, see [Restrict access with column-level access control](/bigquery/docs/column-level-security).
...
To learn more about Data Catalog roles, see [Data Catalog Identity and Access Management (IAM)](/data-catalog/docs/concepts/iam).
To learn more about BigQuery roles, see [Access control with IAM](/bigquery/docs/access-control).
...
- A view that is implicitly authorized to access the tables in a dataset because
it is contained in an authorized dataset.

For more information,
see [Authorized views](/bigquery/docs/authorized-views) and [Authorized datasets](/bigquery/docs/authorized-datasets).
...
![Accessing views](/static/bigquery/images/view-access.png)
...
In legacy SQL, you query historical data by using [time decorators](/bigquery/docs/table-decorators#time_decorators) on the
table name. In GoogleSQL, you query historical data by using the [`FOR SYSTEM_TIME AS OF`](/bigquery/docs/reference/standard-sql/query-syntax#for_system_time_as_of) clause on the table.

Materialized views with the [`max_staleness`](/bigquery/docs/materialized-views-create#max_staleness) option set return historical data
from within their staleness interval. This behavior is similar to a query using [`FOR SYSTEM_TIME AS OF`](/bigquery/docs/reference/standard-sql/query-syntax#for_system_time_as_of) at the time of the view's last refresh, as it
...
relaxed only after [the configured time travel window](/bigquery/docs/time-travel#configure_the_time_travel_window) has passed since the clean-up of the sensitive data.
...
You can create a taxonomy and apply policy tags to tables in [all regions where BigQuery is available](/bigquery/docs/locations#supported_locations).
...
locations, learn more about replicating taxonomies in [Managing policy tags across locations](/bigquery/docs/managing-policy-tags-across-locations#replicating_a_taxonomy_in_a_new_location).
...
- When you migrate a project between organization
resources, policy tag taxonomies don't automatically update. You must
manually recreate taxonomies in the destination
organization to make them visible in the Google Cloud console. For more
information, see [Handle special cases](/resource-manager/docs/handle-special-cases#policy-tag-taxonomies).
...
which features are enabled in each edition, see [Introduction to BigQuery editions](/bigquery/docs/editions-intro).

- BigQuery only supports column-level access control for [BigLake tables](/bigquery/docs/biglake-intro), [BigQuery tables](/bigquery/docs/tables-intro),
and [BigQuery Omni tables](/bigquery/docs/omni-introduction).
...
schema with policy tags. The following example shows how to use `--destination_schema`.
...
If you write query results to a table by specifying the `--destination_table` flag, and the query subsequently raises an exception,
it is possible that any schema changes will be skipped. If this occurs,
check the destination table schema and [manually update it](/bigquery/docs/managing-table-schemas) if necessary.
...
to the lowest-level subtag, as shown in the following screenshot:
  ...
  ![Policy tag depth.](/static/bigquery/images/policy-tag-depth.png)
...
which categories of sensitive data. For more information, see [Auditing policy tags](/bigquery/docs/auditing-policy-tags).
...
- For details about using column-level access control, see [Restrict access with column-level access control](/bigquery/docs/column-level-security).
